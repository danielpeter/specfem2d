...
![image](figures/image0000300.jpg) <div class="figcaption" style="text-align:justify;font-size:80%"><span style="color:#9A9A9A">Figure: image</span></div> ![image](figures/image0000400.jpg) <div class="figcaption" style="text-align:justify;font-size:80%"><span style="color:#9A9A9A">Figure: image</span></div> ![image](figures/image0000500.jpg)
...
- you can find a Fortran code to compute the analytical solution for simple media that we use as a reference in benchmarks in many of our articles at [EX2DDIR](http://www.spice-rtn.org/library/software/EX2DDIR). That code is described in: (Berg et al. 1994)
...
<div class="figcaption" style="text-align:justify;font-size:80%"><span style="color:#9A9A9A">Figure: Example of timing for three sources. The center of the first source triangle is defined to be time zero. Note that this is NOT in general the hypocentral time, or the start time of the source (marked as `tstart`). The time shift parameter `t0` in the `SOURCE` file would be $t1(=0)$, $t2$, $t3$ in this case, and the half-duration parameter, `f0`, would be $\mathtt{hdur1}=1/\mathtt{f0}_1$, $\mathtt{hdur2}=1/\mathtt{f0}_2$, $\mathtt{hdur3}=1/\mathtt{f0}_3$ for the sources 1, 2, 3 respectively.</span></div>
...
CFL upper bound for an elastic simulation with $V_p/V_s = \sqrt{2}$.
